...
matrix_mean2 <- matrix_data2 %>% 
  ...
  filter(!is.na(newID)) %>% 
  ungroup %>% 
  mutate(across(c("this_status", "that_status"), function(x) factor(x, labels = c("Patient", "Visitor", "HCW")))) 
  


  

# how do we actually want to calculate the 
# 1 matrices?
matrix_mean2 %>% 
  ungroup %>% 
...
matrixplot_durmins <- matrix_mean2 %>% 
  ...
  labs(x = "For each X the average contact lasts...", y = "...minutes with each Y", fill = "Minutes")
...
labs(x = "Each X forms new contacts each hour\nwhich cumulatively last...", y = "...with all Y", fill = "Minutes\nper hour")



gtable_show_names(matrixplot_ctcph)
matrixplot_ctcph_repos <- reposition_legend(matrixplot_ctcph, panel = "panel-8-2", position = "top")
matrixplot_durmins_repos <- reposition_legend(matrixplot_durmins, panel = "panel-8-2", position = "top")
matrixplot_prod_repos <- reposition_legend(matrixplot_prod, panel = "panel-8-2", position = "top")

ggsave(matrixplot_ctcph_repos
       , filename = paste0("output/Fig_ctcphmatrix.pdf")
       , units = "cm", width = 40, height = 12, dpi = 100, device = "pdf")

ggsave(matrixplot_durmins_repos
       , filename = paste0("output/Fig_durminsmatrix.pdf")
       , units = "cm", width = 40, height = 12, dpi = 100, device = "pdf")
