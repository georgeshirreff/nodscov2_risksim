...
if(T){ # this if statement included because the analysis can take a long time
...
redDARplot_4status <- DR_pctChange %>% left_join(DR_pctChange_itt_bar) %>%
  ...
  labs(y = "Reduction in number of secondary infections per day per infected individual", x = "High risk individuals targeted by", caption = expression(frac(DAR[baseline]-DAR,DAR[baseline])), shape = "Ward type") #+ 
